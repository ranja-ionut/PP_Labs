...
(define Lista '(2 3 4 5 6 7 8 9 10))
(define (check-bases n b Bases total)
  (if (= total b)
      #t
      (if (empty? Bases)
          #f
          (if (palindrome? (num->base n (car Bases)))
              (check-bases n b (cdr Bases) (+ total 1))
              (check-bases n b (cdr Bases) total)
          )
       )
   )
)
(define (first-b-pal start b)
  (if (check-bases (+ start 1) b Lista 0)
      (+ start 1)
...
(define (check-length L len)
  (if (= len 0)
       null
       (if (palindrome? (take L len))
           (list->num (take L len))
           (if (> (length (cdr L)) 1)
               (if (palindrome? (take (append (cdr L) (list 0)) len))
                   (check-length (cdr L) (- len 1))
                   (check-length L (- len 1))
               )
               null
           )
        )
...
(define (longest-palindrome n) (check-length (num->base n 10) (length (num->base n 10))))
