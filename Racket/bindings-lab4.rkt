...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((a (get-line-segment)) (b (get-start-point a)) (c (get-end-point a)) (d (+ (expt (- (car b) (car c)) 2) (expt (- (cdr b) (cdr c)) 2) )) (e (sqrt d)))
    e
      )
  )
...
(define (distance p1 p2)
  (compute-length (λ () (cons p1 p2)) (λ(x) p1) (λ (x) p2))
  )
...
(define (compute-f-with-step f a b step)
  (let compute ((b b) (result null))
    (if (> a b)
        result
        (compute (- b step) (cons (f b) result))
        )
    )
  )
...
(define (num-concat x y)
  (define (+ x y)
  (let ((a (number->string x)) (b (number->string y)))
