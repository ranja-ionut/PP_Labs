...
safe(_/_/_, []) :- !.
safe(X/Y/S, [_/_/SH|T]) :- S \== SH, safe(X/Y/S, T).
safe(X/Y/S, [XH/YH/SH|T]) :- S == SH, X \== XH, Y \== YH, safe(X/Y/S, T).
...
% dată.
parseState(M/NME/NCE/NMV/NCV, M, NME, NCE, NMV, NCV).
...
next_state(misionari, S1, S2) :-
        parseState(S1, M1, NME1, NCE1, NMV1, NCV1),
        M1 == est, opus(M1, M2),
        boat(X, Y),
        NME2 is NME1 - X,
        NCE2 is NCE1 - Y,
...
preorder(N, _) :- \+arc(N, _).
preorder(N, Parc) :- nod(N), arc(N, Children ),
        member(Z, Children), preorder(Z, [Z|Parc]).
