...
suspect(BetterResults) :- findall(Nume:Marca:Arma, (conduce(Nume, Marca), inarmat(Nume, Arma)), Results), member(BetterResults, Results).
...
au_pusca(ListaNume) :- findall(Nume, (inarmat(Nume, Arma), Arma == pusca), ListaNume).
...
au_arma(Arma, ListaNume) :- findall(Y, inarmat(_, Y), Arme), setof(Nume, (inarmat(Nume, X), member(Arma, Arme), X == Arma), ListaNume).

au_marca(Marca, ListaNume) :- findall(Y, conduce(_, Y), Marci), setof(Nume, (conduce(Nume, X), member(Marca, Marci), X == Marca), ListaNume).
...
arme_bmw(ListaArme) :- findall(Arma, (conduce(X, bmw), inarmat(X, Arma)), ListaArme).
...
arme_marca(Marca, ListaArme) :- au_marca(Marca, ListaNume), findall(Arma, (member(X, ListaNume), inarmat(X, Arma)), ListaArme).
...
marci_arma_unica(ListaMarci) :- findall(Marca, (arme_marca(Marca, ListaArme), has_one(ListaArme)), ListaMarci).
...
suspect1(ListaSuspecti1) :-
        findall(Nume:Marca:Arma, (suspect(Nume:Marca:Arma), arme_marca(Marca, Arme), forall(member(A, Arme), (au_arma(A, X), length(X, L), L > 1))), ListaSuspecti), member(ListaSuspecti1, ListaSuspecti).
...
suspect2(ListaSuspecti2) :-
        findall(Nume:Marca:Arma, (suspect1(Nume:Marca:Arma), au_marca(Marca, Pers), length(Pers, L), L > 1), ListaSuspecti), member(ListaSuspecti2, ListaSuspecti).
...
suspect3(ListaSuspecti3) :-
        findall(Nume:Marca:Arma, (suspect2(Nume:Marca:Arma), au_arma(Arma, Pers), length(Pers, L), L > 1), ListaSuspecti), member(ListaSuspecti3, ListaSuspecti).
