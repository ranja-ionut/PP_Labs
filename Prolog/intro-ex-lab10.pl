...
myConcat([],L,L).
myConcat(L,[],L).
myConcat([H1|T1],L2,[H3|T3]) :- H1 = H3, myConcat(T1,L2,T3).

check1:- writeln('Exercitiul 1'),
    tests([
        exp('myConcat([], [], L)', ['L', []]),
...
myReverse([],[]).
myReverse([H|T], Reverse) :- myReverse(T, Interm), myConcat(Interm, [H], Reverse).
...
myReverseAcc([],[],[]).
myReverseAcc([],L,L).
myReverseAcc([H|T],Acc,Reverse) :- myReverseAcc(T, [H|Acc], Reverse).
...
extract(List, Start, End, List) :-
    Start < End, length(List, L), L1 is L - 1,
    End >= L1, Start =:= 0.
extract([H|T], Start, End, Range) :-
    Start < End, Start > 0,
    length([H|T], L), L1 is L - 1,
    End >= L1, Start1 is Start - 1,
    extract(T, Start1, End, Range).
extract([H|T], Start, End, Range) :-
    Start < End, length([H|T], L), L1 is L - 1,
    End =< L1, Start > 0, Start1 is Start - 1,
    extract(T, Start1, End, Range).
extract(List, Start, End, Range) :-
    Start < End, length(List, L), L1 is L - 1,
    Start =:= 0, End < L1,
    myConcat(Range, Dif, List),
    length(Dif, L2),  L3  is  L1 - End,
    L2 =:= L3.
...
palindrom(List) :- myReverseAcc(List, [], Rev), List = Rev.
...
sublista(_, []).
sublista([H|T], [HS|TS]) :- H = HS, sublista(T, TS).
sublista([_|T], [HS|TS]) :- sublista(T, [HS|TS]).
...
descendantOf(X,Y) :- nod(X), nod(Y), nod(Z),
    arc(Z, X), descendantOf(Z, Y).
...
sameTree(X, Y) :- descendantOf(X, Y).
sameTree(X, Y) :- descendantOf(Y, X).
sameTree(X, Y) :- descendantOf(X, Z), arc(Z, T), descendantOf(Y,T).
...
drum(Nod, Nod, []).
drum(X, Y, [_,_]) :- arc(X, Y).
drum(X, Y, [H|T]) :- descendantOf(X, Y), drum(H, Y, T).
