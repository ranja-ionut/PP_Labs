...
unzip2  :: [(a, b)] -> ([a], [b])
unzip2 [] = ([], [])
unzip2  ((a, b) : rest) = (a : (fst continua), b : (snd continua))
					where continua = (unzip rest)
...
-}
primes :: Int -> [Int] 
primes n = [x | x <- [2..n], [z | z <- [2..(x - 1)], (rem x z) == 0] == []]
...
setDiff :: Eq a => [a] -> [a] -> [a]
setDiff a b = [x | x <- a, [y | y <- b, x == y] == [] ]

cartProduct :: [a] -> [b] -> [(a, b)]
cartProduct a b = [(x, y)| x <- a, y <-b]

setUnion :: Eq a => [a] -> [a] -> [a]
setUnion a b = [x | x <- ((setDiff a b) ++ b)]
...
group2 :: Eq a => [a] -> [[a]]
group2 lista = foldr auxf [[]] lista

auxf x lista
	| lista == [[]] = [[x]] 
	| (head  (head lista)) == x = [((head lista) ++ [x])] ++ (tail lista)
	| otherwise = [[x]] ++ lista
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc lista = foldr auxf2 [] (sort lista)

auxf2 x lista
	| lista == [] = [(x, 1)]
	| (fst (head lista)) == x = [( x , (snd (head lista)) + 1 )] ++ (tail lista)
	| otherwise = [(x, 1)] ++ lista
...
-}
dup :: String -> String
dup input = (unwords (auxf3 (words input)))

auxf3 [] = []
auxf3 (x : xs) = [x] ++ [x] ++ (auxf3 xs)
...
isIsogram input = auxf4 (nrOcc (filter isLetter input))

auxf4 lista
	| lista == [] = True
	| (snd (head lista)) > 1 = False
	| otherwise = auxf4 (tail lista)  
...
-}
diamond :: Char -> [String]
diamond ch =  (reverse (tail text)) ++ text
		where text = (generateList ch 0 ( 2 * ((ord ch) - 65)) ( 2 * ((ord ch) - 65)))

generateList ch left right size
	| left == right = [(replicate (size - left) ' ') ++ [ch] ++ (replicate (size - right) ' ')]
	| otherwise = [(replicate left ' ') ++ [ch] ++ (replicate (size - (2 * left) - 1) ' ') ++ [ch] ++ (replicate (size - right) ' ')] ++ (generateList (pred ch) (left + 1) (right - 1) size)
