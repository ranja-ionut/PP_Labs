...
class (Ord a) => PQueue pq a where
    ...
    fromList list
		| length list == 0 = empty
		| otherwise = foldr insert empty list
    ...
    size :: pq a -> Int
    size pqueue = length $ toList pqueue
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert t1@(prio, x) list@(LPQ l)
		| isEmpty list == True = LPQ [(prio, x)]
		| prio >= (fst $ head l) = LPQ ([t1] ++ l)
		| otherwise = LPQ ([(head l)] ++ (toList $ insert t1 (LPQ (tail l))))

    top l@(LPQ list)
		| isEmpty l == True = Nothing
		| otherwise = Just $ head list

    pop (LPQ list)
		| isEmpty (LPQ list) == True = empty
		| otherwise = LPQ (tail list)
...
merge (Empty 0) (Empty 0) = Empty 0
merge tree (Empty 0) = tree
merge (Empty 0) tree = tree

-- am tradus algoritmul de mai sus de la -> Referință - pentru mai multe detalii despre construcție: http://typeocaml.com/2015/03/12/heap-leftist-tree/  
merge t1@(Node _ val1 l r) t2@(Node _ val2 _ _)
  | (fst val1) < (fst val2) = merge t2 t1
  | otherwise = 
    let merged = (merge r t2)
        rk_left = (rank l)
        rk_right = (rank merged)
    in if (rk_left >= rk_right) then (Node (rk_left + 1) val1 l merged) else (Node (rk_left + 1) val1 merged l)

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty 0) = []
inorder (Node _ val (Empty 0) (Empty 0)) = [val]
inorder tree = (inorder $ left tree) ++ [nodeVal tree] ++ (inorder $ right tree)
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    isEmpty (Empty 0) = True
    isEmpty _ = False
    
    insert (p, x) tree = merge (Node 0 (p, x) (Empty 0) (Empty 0)) tree

    top tree
		| isEmpty tree == True = Nothing
		| otherwise = Just (nodeVal tree)

    pop tree
		| isEmpty tree == True = (Empty 0)
		| otherwise = merge (left tree) (right tree)
...
instance (Show a) => Show (LeftistPQ a) where
    show tree = auxf tree 1

auxf (Empty 0) n = (concat (replicate n "--")) ++ "empty\n"
auxf t@(Node _ val l r) n = (concat (replicate n "--")) ++ show(val) ++ "\n" ++ (auxf l (n + 1)) ++ (auxf r (n + 1))
