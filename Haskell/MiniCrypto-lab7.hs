...
arithmetic init r = prog_arit init
			where prog_arit x = x : prog_arit (x + r)
geometric :: Num a => a -> a -> [a]
geometric init r = prog_geo init
			where prog_geo x = x : prog_geo (x * r)
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms x = generate (next $ mkStdGen x)
		where
			nextgen x = next $ snd x
			generate x = [fromIntegral $ (fst $ x) `mod` 256] ++ (generate (nextgen x))
...
wordToAlpha x = chr $ ((fromIntegral x) `mod` 26) + (ord 'a')
{-
    5. (1p)
...
tableToFunc x = find x
			where find x y 
					| x == [] = y
					| y == (fst $ head x) = (snd $ head x) 
					| otherwise = find (tail x) y
...
{-
	...
	Nu am mai făcut și bonusurile pentru că am o temă neterminată
	și aș vrea să mă concentrez mai mult pe rezolvarea ei.
	
	Aș aprecia totuși dacă la corectarea labului aș primi un feedback
	despre ce lucruri aș fi putut face mai eficient sau mai simplu.
